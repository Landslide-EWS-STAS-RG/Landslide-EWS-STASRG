...
// ======================================================
// NETWORK TELEMETRY (WiFi + MQTT)
// ======================================================
// The ESP32 publishes the same fields the old serial bridge sent
// (tilt, gravity, rotation, piezo, hazard, state), so Node-RED can reuse its
// existing "Route to Dashboard" and "Format for InfluxDB" logic with an
// "mqtt in" node instead of "http in". Edit the values below before flashing.
...
const char *TOPIC_TELEMETRY = "landslide/ews-esp32-01/telemetry";
const char *TOPIC_EVENT     = "landslide/ews-esp32-01/event";
const char *TOPIC_STATUS    = "landslide/ews-esp32-01/status";
...
const uint32_t TELEMETRY_PUBLISH_INTERVAL_MS = 1000;
...
void maintainNetwork() {
  ...
  if (!mqttClient.connected()) {
    if (lastMqttAttemptMs == 0 || tNowMs - lastMqttAttemptMs >= MQTT_RETRY_INTERVAL_MS) {
      ...
      // Last Will: the broker retains "offline" if the device drops out.
...
void publishNetworkTelemetry(const DataRecord &record) {
  ...
  if (!hasNewEvent &&
      tNowMs - lastTelemetryPublishMs < TELEMETRY_PUBLISH_INTERVAL_MS) {
    return;
  }
  lastTelemetryPublishMs = tNowMs;

  JsonDocument doc; // ArduinoJson v7
  doc["device"]     = DEVICE_ID;
  doc["ts"]         = record.timestampMs;
  doc["tilt"]       = record.tiltDeg;
  doc["gravity"]    = record.gravityDiffG;
  doc["rotation"]   = record.rotationDps;
  doc["piezo"]      = record.vibrationIndex;
  ...
  mqttClient.publish(TOPIC_TELEMETRY, (const uint8_t *)payload, len, false);

  if (hasNewEvent) {
    JsonDocument evt;
    evt["device"]    = DEVICE_ID;
    evt["ts"]        = record.timestampMs;
    evt["newEvents"] = record.newEventMask;
    evt["state"]     = stateName(record.state);
    char evtPayload[192];
    size_t evtLen = serializeJson(evt, evtPayload, sizeof(evtPayload));
    mqttClient.publish(TOPIC_EVENT, (const uint8_t *)evtPayload, evtLen, false);
  }
}
